...
httpServer.listenFuture().onComplete{httpServerFuture.completer()}

var netServerFuture = Future.future()
netServer.listenFuture().onComplete{netServerFuture.completer()}
...
fs.createFileFuture("/foo").onComplete{fut1.completer()}

fut1.compose((v: java.lang.Void) => {
  // When the file is created (fut1), execute this:
  var fut2 = Future.future()
  fs.writeFileFuture("/foo", Buffer.buffer()).onComplete{fut2.completer()}
  return fut2
}).compose((v: java.lang.Void) => {
  // When the file is written (fut2), execute this:
  fs.moveFuture("/foo", "/bar").onComplete{startFuture.completer()}
}, startFuture)
