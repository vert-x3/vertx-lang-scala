...
val vertx = Vertx.vertx(new io.vertx.core.json.JsonObject(Map(
  "workerPoolSize" -> 40
).asJava))
...
The verticle name is used to look up the specific `link:scaladocs/io/vertx/scala/core/spi/VerticleFactory.html[VerticleFactory]` that will be used to
...
vertx.deployVerticle("com.mycompany.MyOrderProcessorVerticle") { res =>
  if (res.isSuccess) {
    println("Deployment id is: " + res.get)
  } else {
...
----
import scala.collection.JavaConverters._
var config = new io.vertx.core.json.JsonObject(Map(
  "name" -> "tim",
  "directory" -> "/blah"
).asJava)
var options = new io.vertx.core.json.JsonObject(Map(
...
`link:scaladocs/io/vertx/scala/core/spi/metrics/VertxMetrics.html[API Documentation]`.
