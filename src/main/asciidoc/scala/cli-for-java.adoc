...
and `TypedArgument` in the `link:../../scaladoc/io/vertx/scala/core/cli/CLI.html[CLI]` definition:
...
Once annotated, you can define the `link:../../scaladoc/io/vertx/scala/core/cli/CLI.html[CLI]` and inject the values using:
