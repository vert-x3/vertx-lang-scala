...
class WebSocketStream(private val _asJava: io.vertx.core.http.WebSocketStream) 
  ...
  def exceptionHandler(handler: scala.Option[Throwable => Unit]): io.vertx.scala.core.http.WebSocketStream = {
  ...
  def handler(handler: scala.Option[io.vertx.scala.core.http.WebSocket => Unit]): io.vertx.scala.core.http.WebSocketStream = {
    _asJava.handler(funcToMappedHandler(WebSocket.apply)((if(handler.isDefined) handler.get else null)))
    this
  }
  ...
  def endHandler(endHandler: scala.Option[() => Unit]): io.vertx.scala.core.http.WebSocketStream = {
