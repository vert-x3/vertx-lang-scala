...
class MqttEndpoint(private val _asJava: Object) {
  ...
  private var cached_3: Option[MqttAuth] = None
  private var cached_4: Option[MqttWill] = None
  private var cached_5: Option[Int] = None
  ...
  def remoteAddress(): SocketAddress = {
    if (cached_0 == None) {
      val tmp = asJava.asInstanceOf[JMqttEndpoint].remoteAddress()
  ...
  def localAddress(): SocketAddress = {
    if (cached_1 == None) {
      val tmp = asJava.asInstanceOf[JMqttEndpoint].localAddress()
  ...
  /**
   * @return the Authentication information as provided by the remote MQTT clientsee <a href="../../../../../../cheatsheet/MqttAuth.html">MqttAuth</a>
   */
  def auth(): MqttAuth = {
    if (cached_3 == None) {
      val tmp = asJava.asInstanceOf[JMqttEndpoint].auth()
      cached_3 = Some(MqttAuth(tmp))
    }
    cached_3.get
  }

  /**
   * @return the Will information as provided by the remote MQTT clientsee <a href="../../../../../../cheatsheet/MqttWill.html">MqttWill</a>
   */
  def will(): MqttWill = {
    if (cached_4 == None) {
      val tmp = asJava.asInstanceOf[JMqttEndpoint].will()
      cached_4 = Some(MqttWill(tmp))
    }
    cached_4.get
  }
  ...
  def protocolVersion(): Int = {
    if (cached_5 == None) {
      ...
      cached_5 = Some(tmp.asInstanceOf[Int])
    }
    cached_5.get
  }
  ...
  def keepAliveTimeSeconds(): Int = {
    if (cached_8 == None) {
      ...
      cached_8 = Some(tmp.asInstanceOf[Int])
    }
    cached_8.get
  }
  ...
  def lastMessageId(): Int = {
    if (cached_9 == None) {
      ...
      cached_9 = Some(tmp.asInstanceOf[Int])
    }
    cached_9.get
  }
   ...
   * message is received by the remote MQTT client   * @param handler the handler
   * @return a reference to this, so the API can be used fluently
   */
   ...
   * PINGRESP internally after executing this handler.   * @param handler the handler
   * @return a reference to this, so the API can be used fluently
   */
  ...
  def accept(): MqttEndpoint = {
    asJava.asInstanceOf[JMqttEndpoint].accept()
    this
  }

  /**
   * Sends the CONNACK message to the remote MQTT client with "connection accepted"
   * return code. See [[io.vertx.scala.mqtt.MqttEndpoint#reject]] for refusing connection   * @param sessionPresent if a previous session is present
   * @return a reference to this, so the API can be used fluently
   */
   ...
   * request with specified return code. See [[io.vertx.scala.mqtt.MqttEndpoint#accept]] for accepting connection   * @param returnCode the connect return code
   * @return a reference to this, so the API can be used fluently
   */
  ...
  /**
   * Sends the PUBLISH message to the remote MQTT server explicitly specifying the messageId   * @param topic topic on which the message is published
   * @param payload message payload
   * @param qosLevel QoS level
   * @param isDup if the message is a duplicate
   * @param isRetain if the message needs to be retained
  ...
  /**
   * @return true if this io.vertx.scala.mqtt.MqttEndpoint is encrypted via SSL/TLS.
   */
  def isSsl (): Boolean = {
    asJava.asInstanceOf[JMqttEndpoint].isSsl().asInstanceOf[Boolean]
  }
  ...
  * Like [[publish]] but returns a [[scala.concurrent.Future]] instead of taking an AsyncResultHandler.
  */
  def publishFuture (topic: String, payload: io.vertx.core.buffer.Buffer, qosLevel: io.netty.handler.codec.mqtt.MqttQoS, isDup: Boolean, isRetain: Boolean, messageId: Int): scala.concurrent.Future[Int] = {
    //TODO: https://github.com/vert-x3/vertx-codegen/issues/111
    val promiseAndHandler = handlerForAsyncResultWithConversion[java.lang.Integer, Int](x => x.asInstanceOf[Int])
    asJava.asInstanceOf[JMqttEndpoint].publish(topic.asInstanceOf[java.lang.String], payload, qosLevel, isDup.asInstanceOf[java.lang.Boolean], isRetain.asInstanceOf[java.lang.Boolean], messageId.asInstanceOf[java.lang.Integer], promiseAndHandler._1)
    promiseAndHandler._2.future
  }
