...
route.handler((routingContext: io.vertx.scala.ext.web.RoutingContext) => {
  ...
  var productType = routingContext.pathParam("productType")
  var productID = routingContext.pathParam("productID")
...
route.pathRegex("\\/([^\\/]+)\\/([^\\/]+)").handler((routingContext: io.vertx.scala.ext.web.RoutingContext) => {
  ...
  var productType = routingContext.pathParam("param0")
  var productID = routingContext.pathParam("param1")
...
var route = router.route().method(HttpMethod.valueOf("MKCOL")).handler((routingContext: io.vertx.scala.ext.web.RoutingContext) => {
...
[source,scala]
----
router.route().virtualHost("*.vertx.io").handler((routingContext: io.vertx.scala.ext.web.RoutingContext) => {
...
[source,scala]
----
rc.attachment("weekly-report.pdf").end(pdfBuffer)
...
rc.json(new io.vertx.core.json.JsonObject().put("hello", "vert.x"))
// also applies to arrays
rc.json(new io.vertx.core.json.JsonArray().add("vertx").add("web"))
...
// create an OAuth2 provider, clientID and clientSecret should be requested to Google
var authProvider = OAuth2Auth.create(vertx, OAuth2ClientOptions()
  .setClientID("CLIENT_ID")
  .setClientSecret("CLIENT_SECRET")
  .setFlow(OAuth2FlowType.AUTH_CODE)
  .setSite("https://accounts.google.com")
...
oauth2.extraParams(new io.vertx.core.json.JsonObject().put("scope", "profile"))
...
router.route("/protected").handler(OAuth2AuthHandler.create(vertx, authProvider).setupCallback(router.route("/callback")).extraParams(new io.vertx.core.json.JsonObject().put("scope", "user:email")))
...
router.route(HttpMethod.GET, "/").handler((ctx: io.vertx.scala.ext.web.RoutingContext) => {
  // do GET stuff...
})

router.route(HttpMethod.POST, "/").handler((ctx: io.vertx.scala.ext.web.RoutingContext) => {
  // do POST stuff...
})
