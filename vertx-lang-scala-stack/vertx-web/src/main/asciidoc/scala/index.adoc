...
route.handler((ctx: io.vertx.scala.ext.web.RoutingContext) => {
  ...
  ctx.vertx().setTimer(5000, (tid: java.lang.Long) => {
    ctx.next()
  })
})
...
router.route().blockingHandler((ctx: io.vertx.scala.ext.web.RoutingContext) => {
...
`link:../../scaladocs/io/vertx/scala/ext/web/RoutingContext.html#pathParam()[pathParam]`.
...
route.pathRegex("\\/([^\\/]+)\\/([^\\/]+)").handler((ctx: io.vertx.scala.ext.web.RoutingContext) => {

  var productType = ctx.pathParam("param0")
  var productID = ctx.pathParam("param1")
...
router.routeWithRegex("\\/(?<productType>[^\\/]+)\\/(?<productID>[^\\/]+)").handler((ctx: io.vertx.scala.ext.web.RoutingContext) => {
...
router.get("/some/path/").handler((ctx: io.vertx.scala.ext.web.RoutingContext) => {
...
router.getWithRegex(".*foo").handler((ctx: io.vertx.scala.ext.web.RoutingContext) => {
...
router.route("/some/path/").handler((ctx: io.vertx.scala.ext.web.RoutingContext) => {
...
router.route("/some/path/").order(1).handler((ctx: io.vertx.scala.ext.web.RoutingContext) => {
...
router.route("/some/path/").order(0).handler((ctx: io.vertx.scala.ext.web.RoutingContext) => {
...
router.route("/some/path/").order(2).handler((ctx: io.vertx.scala.ext.web.RoutingContext) => {
...
// Exact match
router.route().consumes("text/html").handler((ctx: io.vertx.scala.ext.web.RoutingContext) => {
...
router.route().consumes("text/html").consumes("text/plain").handler((ctx: io.vertx.scala.ext.web.RoutingContext) => {
...
router.route().consumes("text/*").handler((ctx: io.vertx.scala.ext.web.RoutingContext) => {
...
router.route().consumes("*/json").handler((ctx: io.vertx.scala.ext.web.RoutingContext) => {
...
router.route().produces("application/json").handler((ctx: io.vertx.scala.ext.web.RoutingContext) => {
...
router.route().produces("application/json").produces("text/html").handler((ctx: io.vertx.scala.ext.web.RoutingContext) => {

  var response = ctx.response()

  // Get the actual MIME type acceptable
  var acceptableContentType = ctx.getAcceptableContentType()
...
[source,scala]
----
router.route().virtualHost("*.vertx.io").handler((ctx: io.vertx.scala.ext.web.RoutingContext) => {
...
router.get("/some/path/other").handler((ctx: io.vertx.scala.ext.web.RoutingContext) => {
...
Alternatively you can access the entire context data map with `link:../../scaladocs/io/vertx/scala/ext/web/RoutingContext.html#data()[data]`.
...
[source,scala]
----
ctx.attachment("weekly-report.pdf").end(pdfBuffer)
...
ctx.json(new io.vertx.core.json.JsonObject().put("hello", "vert.x"))
// also applies to arrays
ctx.json(new io.vertx.core.json.JsonArray().add("vertx").add("web"))
...
router.get("/some/path/B").handler((ctx: io.vertx.scala.ext.web.RoutingContext) => {
  ctx.response().end()
})
...
restAPI.get("/products/:productID").handler((ctx: io.vertx.scala.ext.web.RoutingContext) => {
...
restAPI.put("/products/:productID").handler((ctx: io.vertx.scala.ext.web.RoutingContext) => {
...
restAPI.delete("/products/:productID").handler((ctx: io.vertx.scala.ext.web.RoutingContext) => {
...
var route = router.get("/localized").handler((ctx: io.vertx.scala.ext.web.RoutingContext) => {
  ...
  ctx.acceptableLanguages().foreach(language => {
    return
  })

  // we do not know the user language so lets just inform that back:
  ctx.response().end(s"Sorry we don't speak: ${ctx.preferredLanguage()}")
})

----

The main method `link:../../scaladocs/io/vertx/scala/ext/web/RoutingContext.html#acceptableLanguages()[acceptableLanguages]` will return the ordered list of locales the
user understands, if you're only interested in the user prefered locale then the helper:
`link:../../scaladocs/io/vertx/scala/ext/web/RoutingContext.html#preferredLanguage()[preferredLanguage]` will return the 1st element of the list or `null` if no
...
route.failureHandler((ctx: io.vertx.scala.ext.web.RoutingContext) => {
...
route1.handler((ctx: io.vertx.scala.ext.web.RoutingContext) => {
...
router.post("/some/path/uploads").handler((ctx: io.vertx.scala.ext.web.RoutingContext) => {
...
var someCookie = ctx.getCookie("mycookie")
...
var chain = ChainAuthHandler.any().add(authNHandlerA).add(ChainAuthHandler.all().add(authNHandlerB).add(authNHandlerC))
...
router.route("/somepath/blah/").handler((ctx: io.vertx.scala.ext.web.RoutingContext) => {
...
To create an auth handler you need an instance of `link:../../scaladocs/io/vertx/scala/ext/auth/authentication/AuthenticationProvider.html[AuthenticationProvider]`.
...
router.route("/someotherpath").handler((ctx: io.vertx.scala.ext.web.RoutingContext) => {

  // This will be public access - no login required

})

router.route("/private/somepath").handler((ctx: io.vertx.scala.ext.web.RoutingContext) => {

  // This will require a login

  // This will have the value true
  var isAuthenticated = ctx.user() != null

})

----

If the authentication handler has successfully authenticated the user it will inject a `link:../../scaladocs/io/vertx/scala/ext/auth/User.html[User]`
...
When the basic auth handler receives this information, it calls the configured `link:../../scaladocs/io/vertx/scala/ext/auth/authentication/AuthenticationProvider.html[AuthenticationProvider]`
...
var jwt = JWTAuth.create(vertx, authConfig)
...
router.route("/login").handler((ctx: io.vertx.scala.ext.web.RoutingContext) => {
  ...
  if ("paulo" == ctx.request().getParam("username") && "secret" == ctx.request().getParam("password")) {
    ctx.response().end(jwt.generateToken(new io.vertx.core.json.JsonObject().put("sub", "paulo")))
  } else {
    ctx.fail(401)
  }
})
...
var handler = (ctx: io.vertx.scala.ext.web.RoutingContext) => {
  var theSubject = ctx.user().principal().getValue("sub")
  var someKey = ctx.user().principal().getValue("someKey")
}
...
`link:../../scaladocs/io/vertx/scala/ext/web/handler/AuthorizationHandler.html[AuthorizationHandler]` should be used. An authorization handler will load all known
authorizations from a given `link:../../scaladocs/io/vertx/scala/ext/auth/authorization/AuthorizationProvider.html[AuthorizationProvider]`.

[source,scala]
----
// attest that all requests on the route match the required authorization
router.route().handler(AuthorizationHandler.create(PermissionBasedAuthorization.create("can-do-work")).addAuthorizationProvider(authProvider))
...
router.route("/listproducts/*").handler(AuthorizationHandler.create(PermissionBasedAuthorization.create("list_products")).addAuthorizationProvider(authProvider))

// Only "admin" has access to /private/settings
router.route("/private/settings/*").handler(AuthorizationHandler.create(RoleBasedAuthorization.create("admin")).addAuthorizationProvider(authProvider))
...
* The `link:../../scaladocs/io/vertx/scala/ext/web/handler/WebAuthnHandler.html[WebAuthnHandler]` requires `link:../../scaladocs/io/vertx/scala/ext/auth/webauthn/WebAuthn.html[WebAuthn]`.
...
var githubOAuth2 = OAuth2AuthHandler.create(vertx, gitHubAuthProvider, "https://myserver.com/github-callback")
...
var googleAuthProvider = OAuth2Auth.create(vertx, OAuth2Options()
...
var googleOAuth2 = OAuth2AuthHandler.create(vertx, googleAuthProvider, "http://localhost:8080")
...
Template engines are described by `link:../../scaladocs/io/vertx/scala/ext/web/common/template/TemplateEngine.html[TemplateEngine]`. In order to render a template
...
router.get("/dynamic").handler((ctx: io.vertx.scala.ext.web.RoutingContext) => {
...
[source,scala]
----
router.get("/api/books").produces("application/json").handler((ctx: io.vertx.scala.ext.web.RoutingContext) => {
  findBooks().onSuccess((books: java.util.List<examples.WebExamples.Book>) => {
    ctx.response().putHeader("Content-Type", "application/json").end(toJson(books))
  }).onFailure(ctx.fail _)
})
...
router.get("/api/books").produces("text/xml").produces("application/json").handler((ctx: io.vertx.scala.ext.web.RoutingContext) => {
  findBooks().onSuccess((books: java.util.List<examples.WebExamples.Book>) => {
    if (ctx.getAcceptableContentType() == "text/xml") {
...
Each match is a `link:../../vertx-bridge-common/dataobjects.html#PermittedOptions[PermittedOptions]` object:
...
var oauth2 = OAuth2AuthHandler.create(vertx, authProvider, "https://myserver.com/callback")
...
// create an OAuth2 provider, clientID and clientSecret
// should be requested to Google
var authProvider = OAuth2Auth.create(vertx, OAuth2Options()
  .setClientID("CLIENT_ID")
  .setClientSecret("CLIENT_SECRET")
  .setFlow(OAuth2FlowType.AUTH_CODE)
  .setSite("https://accounts.google.com")
...
// create a oauth2 handler on our domain: "http://localhost:8080"
var oauth2 = OAuth2AuthHandler.create(vertx, authProvider, "http://localhost:8080")
...
var oauth2 = OAuth2AuthHandler.create(vertx, provider, "https://myserver.com:8447/callback")
...
var authProvider = GithubAuth.create(vertx, "CLIENTID", "CLIENT SECRET")
// We need a user session handler too to make sure
// the user is stored in the session between requests
router.route().handler(SessionHandler.create(LocalSessionStore.create(vertx)))
// we now protect the resource under the path "/protected"
router.route("/protected").handler(OAuth2AuthHandler.create(vertx, authProvider).setupCallback(router.route("/callback")).withScope("user:email"))
...
var webAuthn = WebAuthn.create(vertx, WebAuthnOptions()
  .setOrigin("https://192.168.178.74.xip.io:8443")
  .setRelayParty(RelayParty()
...
// security handler
var webAuthNHandler = WebAuthnHandler.create(webAuthn).setupCallback(router.post("/webauthn/response")).setupCredentialsCreateCallback(router.post("/webauthn/register")).setupCredentialsGetCallback(router.post("/webauthn/login"))
