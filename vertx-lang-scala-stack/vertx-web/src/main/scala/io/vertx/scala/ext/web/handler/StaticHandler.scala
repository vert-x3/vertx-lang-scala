...
class StaticHandler(private val _asJava: Object) extends io.vertx.core.Handler[RoutingContext] {
  def asJava = _asJava
   ...
   * `Content-Encoding` header set to `identity` for the types present in the `mediaTypes` set   * @param mediaTypes the set of mime types that are already compressed
   * @return a reference to this, so the API can be used fluently
   */
  
  def skipCompressionForMediaTypes(mediaTypes: scala.collection.mutable.Set[String]): StaticHandler = {
    asJava.asInstanceOf[JStaticHandler].skipCompressionForMediaTypes(mediaTypes.map(x => x.asInstanceOf[java.lang.String]).asJava)
    this
  }

  /**
   * Skip compression if the suffix of the file to send is in the provided `fileSuffixes` set.
   * `Content-Encoding` header set to `identity` for the suffixes present in the `fileSuffixes` set   * @param fileSuffixes the set of file suffixes that are already compressed
   * @return a reference to this, so the API can be used fluently
   */
  
  def skipCompressionForSuffixes(fileSuffixes: scala.collection.mutable.Set[String]): StaticHandler = {
    asJava.asInstanceOf[JStaticHandler].skipCompressionForSuffixes(fileSuffixes.map(x => x.asInstanceOf[java.lang.String]).asJava)
    this
  }
