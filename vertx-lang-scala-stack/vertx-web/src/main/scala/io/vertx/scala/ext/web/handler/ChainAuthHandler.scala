...
class ChainAuthHandler(private val _asJava: Object) extends AuthHandler with io.vertx.core.Handler[RoutingContext] {
  def asJava = _asJava
  ...
  /**
   * Appends a auth provider to the chain.   * @param authHandler auth handler
   * @return self
   */
  ...
  /**
   * Removes a provider from the chain.   * @param authHandler provider to remove
