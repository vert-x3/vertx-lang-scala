...
`link:../../scaladocs/io/vertx/scala/ext/auth/authorization/AuthorizationProvider.html[AuthorizationProvider]` which uses the Vert.x `link:../../scaladocs/io/vertx/scala/ext/jdbc/JDBCClient.html[JDBCClient]`
...
Once you've got one of those you can create a `link:../../scaladocs/io/vertx/scala/ext/auth/jdbc/JDBCAuthentication.html[JDBCAuthentication]` instance as follows:

[source,scala]
----

var jdbcClient = JDBCClient.createShared(vertx, jdbcClientConfig)
var options = JDBCAuthenticationOptions()

var authenticationProvider = JDBCAuthentication.create(jdbcClient, options)
...
`link:../dataobjects.html#JDBCAuthenticationOptions#setAuthenticationQuery()[authenticationQuery]`,
`link:../dataobjects.html#JDBCAuthorizationOptions#setPermissionsQuery()[permissionsQuery]` and
`link:../dataobjects.html#JDBCAuthorizationOptions#setRolesQuery()[rolesQuery]`, if you want to use them with a different database schema.

The `link:../../scaladocs/io/vertx/scala/ext/auth/jdbc/JDBCHashStrategy.html[JDBCHashStrategy]` is a required parameter to build an instance of `link:../../scaladocs/io/vertx/scala/ext/auth/jdbc/JDBCAuthorization.html[JDBCAuthorization]`
...
THe legacy hashing strategy can be used on the *deprecated* factory method of JDBCAuthentication `link:../../scaladocs/io/vertx/scala/ext/auth/jdbc/JDBCAuthentication.html#create(io.vertx.ext.jdbc.JDBCClient,%20io.vertx.ext.auth.jdbc.JDBCHashStrategy)[JDBCAuthentication.create]`
...
var hash = jdbcAuth.hash("pbkdf2", VertxContextPRNG.current().nextString(32), "sausages")
// save to the database
conn.updateWithParamsFuture("INSERT INTO user (username, password) VALUES (?, ?)", new io.vertx.core.json.JsonArray().add("tim").add(hash)).onComplete{
  case Success(result) => {
    // success!
  }
...
[source,scala]
----
jdbcAuthZ.getAuthorizations(user).onSuccess((v: java.lang.Void) => {
