...
object KeycloakAuth {
  ...
  def discover(vertx: Vertx,config: OAuth2ClientOptions,handler: Handler[AsyncResult[OAuth2Auth]]): Unit = {
    JKeycloakAuth.discover(vertx.asJava.asInstanceOf[JVertx], config.asJava, {x: AsyncResult[JOAuth2Auth] => handler.handle(AsyncResultWrapper[JOAuth2Auth, OAuth2Auth](x, a => OAuth2Auth(a)))})
  }
