...
var token = provider.generateToken(new io.vertx.core.json.JsonObject().put("some", "token-data"), JWTOptions()
  .setAlgorithm("RS256")
)
...
var token = provider.generateToken(new io.vertx.core.json.JsonObject(), JWTOptions()
  .setAlgorithm("ES256")
)
...
jwtAuth.authenticate(new io.vertx.core.json.JsonObject().put("jwt", "BASE64-ENCODED-STRING")).onSuccess((user: io.vertx.scala.ext.auth.User) => {
...
jwtAuth.authenticate(new io.vertx.core.json.JsonObject().put("jwt", "BASE64-ENCODED-STRING").put("options", new io.vertx.core.json.JsonObject().put("ignoreExpiration", true))).onSuccess((user: io.vertx.scala.ext.auth.User) => {
  println(s"User: ${user.principal()}")
}).onFailure((err: java.lang.Throwable) => {
  // Failed!
})
...
jwtAuth.authenticate(new io.vertx.core.json.JsonObject().put("jwt", "BASE64-ENCODED-STRING").put("options", new io.vertx.core.json.JsonObject().put("audience", new io.vertx.core.json.JsonArray().add("paulo@server.com")))).onSuccess((user: io.vertx.scala.ext.auth.User) => {
  println(s"User: ${user.principal()}")
}).onFailure((err: java.lang.Throwable) => {
  // Failed!
})
...
jwtAuth.authenticate(new io.vertx.core.json.JsonObject().put("jwt", "BASE64-ENCODED-STRING").put("options", new io.vertx.core.json.JsonObject().put("issuer", "mycorp.com"))).onSuccess((user: io.vertx.scala.ext.auth.User) => {
  println(s"User: ${user.principal()}")
}).onFailure((err: java.lang.Throwable) => {
  // Failed!
})
...
user has a specific authority. Authorization will follow the common `link:../../scaladocs/io/vertx/scala/ext/auth/authorization/AuthorizationProvider.html[AuthorizationProvider]`
API. Choose the provider that generated your token and evaluate.

Currently there are 2 factories:

* `link:../../scaladocs/io/vertx/scala/ext/auth/jwt/authorization/JWTAuthorization.html[JWTAuthorization]` inspects tokens based on the "permissions" claim key.
* `link:../../scaladocs/io/vertx/scala/ext/auth/jwt/authorization/MicroProfileAuthorization.html[MicroProfileAuthorization]` inspects tokens based on the <a href="https://www.eclipse.org/community/eclipse_newsletter/2017/september/article2.php">MP JWT spec</a>.

The typical usage is to extract the permissions form the user object using the provider and perform the attestation:

[source,scala]
----
var authz = MicroProfileAuthorization.create()
...
var provider = JWTAuth.create(vertx, todo-renderToDataObject)
