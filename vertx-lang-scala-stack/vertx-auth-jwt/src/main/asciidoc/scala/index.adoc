...
To create an instance of the provider you use `link:../../scaladoc/io/vertx/scala/ext/auth/jwt/JWTAuth.html[JWTAuth]`. You specify the configuration
in a JSON object.
...
var config = new io.vertx.core.json.JsonObject().put("keyStore", new io.vertx.core.json.JsonObject().put("path", "keystore.jceks").put("type", "jceks").put("password", "secret"))
...
var config = new io.vertx.core.json.JsonObject().put("public-key", "BASE64-ENCODED-PUBLIC_KEY")
var provider = JWTAuth.create(vertx, config)
