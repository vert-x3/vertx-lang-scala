...
class VertxContextPRNG(private val _asJava: Object) {
   ...
   * of the String before the encoding step.   * @param length the desired string length before Base64 encoding.
  ...
  /**
   * Returns a secure random int, between 0 (inclusive) and the specified bound (exclusive).   * @param bound the upper bound (exclusive), which must be positive.
   * @return random int.
   */
