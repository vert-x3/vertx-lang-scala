...
class User(private val _asJava: Object) {
   ...
   * underlying auth provider each time.  Use this method if you want to clear this cache.   * @return the User to enable fluent use
   */
   ...
   *     "username", "tim"
   *   `
   * </pre>   * @return JSON representation of the Principal
   */
   ...
   * after it has been deserialized.   * @param authProvider the AuthProvider - this must be the same type of AuthProvider that originally created the User
   */
