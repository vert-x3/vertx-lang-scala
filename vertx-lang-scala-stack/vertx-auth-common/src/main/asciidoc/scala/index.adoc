...
To authenticate a user you use `link:../../scaladocs/io/vertx/scala/ext/auth/AuthProvider.html#authenticate(io.vertx.core.json.JsonObject)[authenticate]`.
...
to check if a user has a specific authority you use `link:../../scaladocs/io/vertx/scala/ext/auth/User.html#isAuthorised(java.lang.String)[isAuthorised]`.
...
user.isAuthorizedFuture("printers:printer1234").onComplete{
  case Success(result) => {
