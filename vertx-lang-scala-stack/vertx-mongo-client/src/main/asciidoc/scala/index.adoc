...
In that case you can use `link:../../scaladocs/io/vertx/scala/ext/mongo/MongoClient.html#create(io.vertx.core.Vertx)[MongoClient.create]`.

[source,scala]
----
var client = MongoClient.create(vertx, config)
...
----
var document = new io.vertx.core.json.JsonObject().put("title", "The Hobbit")
mongoClient.save("books", document).compose((v: java.lang.String) => {
  mongoClient.distinct("books", "title", java.lang.String.class.getName())
}).onCompleteFuture().onComplete{
  case Success(result) => {
    println(s"Title is : ${result.getValue(0)}")
  }
  case Failure(cause) => {
    println(s"$cause")
  }
...
mongoClient.save("books", document).compose((v: java.lang.String) => {
  mongoClient.distinctWithQuery("books", "title", java.lang.String.class.getName(), query)
}).onCompleteFuture().onComplete{
  case Success(result) => {
    println(s"Title is : ${result.getValue(0)}")
  }
