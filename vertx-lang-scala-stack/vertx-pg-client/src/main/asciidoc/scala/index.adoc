...
[source,scala]
----
client.preparedQueryFuture("INSERT INTO color (color_name) VALUES ($1), ($2), ($3) RETURNING color_id", Tuple.of("white", "red", "blue")).onComplete{
  case Success(result) => {
    var rows = result
...
[source,scala]
----
pool.queryFuture("SELECT 1::BIGINT \"VAL\"").onComplete{
  case Success(result) => println("Success")
  case Failure(cause) => println("Failure")
}
...
[source,scala]
----
client.preparedQueryFuture("SELECT address, (address).city FROM address_book WHERE id=$1", Tuple.of(3)).onComplete{
  case Success(result) => {
    var rows = result
    rows.foreach(row => {
...
[source,scala]
----
client.preparedQueryFuture("INSERT INTO address_book (id, address) VALUES ($1, $2)", Tuple.of(3, "('Anytown', 'Second Ave', false)")).onComplete{
  case Success(result) => {
    var rows = result
    println(rows.rowCount())
  }
...
[source,scala]
----
client.preparedQueryFuture("SELECT to_tsvector( $1 ) @@ to_tsquery( $2 )", Tuple.of("fat cats ate fat rats", "fat & rat")).onComplete{
  case Success(result) => {
    var rows = result
    rows.foreach(row => {
...
connection.queryFuture("LISTEN some-channel").onComplete{
  case Success(result) => println("Success")
  case Failure(cause) => println("Failure")
}
...
subscriber.connectFuture().onComplete{
  case Success(result) => {
    ...
    subscriber.channel("Complex.Channel.Name").subscribeHandler((subscribed: java.lang.Void) => {
      subscriber.actualConnection().queryFuture("NOTIFY \"Complex.Channel.Name\", 'msg'").onComplete{
        case Success(result) => println("Success")
        case Failure(cause) => println("Failure")
      }
    })
    ...
    subscriber.channel("simple_channel").subscribeHandler((subscribed: java.lang.Void) => {
      ...
      subscriber.actualConnection().queryFuture("NOTIFY Simple_CHANNEL, 'msg'").onComplete{
        case Success(result) => println("Success")
        case Failure(cause) => println("Failure")
      }
    })
...
[source,scala]
----
connection.queryFuture("SELECT pg_sleep(20)").onComplete{
  case Success(result) => {
