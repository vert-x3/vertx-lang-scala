...
class HttpResponse[T: TypeTag](private val _asJava: Object) {
  ...
  /**
   * @return the response body decoded as a Buffer, or `null` if a codec other than BodyCodec#buffer() was used
   */
  ...
  /**
   * @return the response body decoded as a `String` given a specific `encoding`, or `null` if a codec other than BodyCodec#buffer() was used
   */
  ...
  /**
   * @return the response body decoded as the specified `type` with the Jackson mapper, or `null` if a codec other than BodyCodec#buffer() was used
   */
