...
consumer.subscribeFuture("test").onComplete{
  case Success(result) => {
    ...
    vertx.setPeriodic(1000, (timerId: java.lang.Long) => {
      ...
      consumer.pollFuture(java.time.Duration.ofMillis(100)).onComplete{
        case Success(result) => {
...
You can use  `link:../../scaladocs/io/vertx/scala/kafka/client/producer/KafkaProducer.html#write()[write]` to send messages (records) to a topic.
