...
topics.add("topic2")
topics.add("topic3")
consumer.subscribe(topics).onSuccess((v: java.lang.Void) => {
  println("subscribed")
}).onFailure((cause: java.lang.Throwable) => {
  println(s"Could not subscribe ${cause.getMessage()}")
})

// or just subscribe to a single topic
consumer.subscribe("a-single-topic").onSuccess((v: java.lang.Void) => {
  println("subscribed")
}).onFailure((cause: java.lang.Throwable) => {
  println(s"Could not subscribe ${cause.getMessage()}")
})
...
consumer.subscribe("test").onSuccess((v: java.lang.Void) => {
  println("subscribed")
}).onFailure((cause: java.lang.Throwable) => {
  println(s"Could not subscribe ${cause.getMessage()}")
})
...
// unsubscribing request
consumer.unsubscribe().onSuccess((v: java.lang.Void) => {
...
consumer.assign(topicPartitions).onSuccess((v: java.lang.Void) => {
  println("Partition assigned")
}).compose((v: java.lang.Void) => {
  consumer.assignment()
}).onSuccess((partitions: java.util.Set<io.vertx.scala.kafka.client.common.TopicPartition>) => {
...
topic, avoiding to register the handler for getting the messages and then using the `link:../../scaladocs/io/vertx/scala/kafka/client/consumer/KafkaConsumer.html#poll(java.time.Duration)[poll]` method.
...
consumer.subscribe("test").onSuccess((v: java.lang.Void) => {
  ...
  vertx.setPeriodic(1000, (timerId: java.lang.Long) => {
    consumer.poll(java.time.Duration.ofMillis(100)).onSuccess((records: io.vertx.scala.kafka.client.consumer.KafkaConsumerRecords<java.lang.String,java.lang.String>) => {
      ...
      println(s"Something went wrong when polling ${cause.toString()}")
...
consumer.partitionsFor("test").onSuccess((partitions: java.util.List<io.vertx.scala.kafka.client.common.PartitionInfo>) => {
...
consumer.listTopics().onSuccess((partitionsTopicMap: java.util.Map<java.lang.String,java.util.List<io.vertx.scala.kafka.client.common.PartitionInfo>>) => {
  partitionsTopicMap.foreach{
...
consumer.commit().onSuccess((v: java.lang.Void) => {
...
consumer.seek(topicPartition, 10).onSuccess((v: java.lang.Void) => {
...
consumer.seekToBeginning(java.util.Collections.singleton(topicPartition)).onSuccess((v: java.lang.Void) => {
  println("Seeking done")
})
...
consumer.seekToEnd(java.util.Collections.singleton(topicPartition)).onSuccess((v: java.lang.Void) => {
  println("Seeking done")
})
...
consumer.beginningOffsets(topicPartitions).onSuccess((results: java.util.Map<io.vertx.scala.kafka.client.common.TopicPartition,java.lang.Long>) => {
  results.foreach{
...
consumer.beginningOffsets(topicPartition).onSuccess((beginningOffset: java.lang.Long) => {
  println(s"Beginning offset for topic=${todo-renderDataObjectMemberSelect}, partition=${todo-renderDataObjectMemberSelect}, beginningOffset=${beginningOffset}")
})
...
consumer.endOffsets(topicPartitions).onSuccess((results: java.util.Map<io.vertx.scala.kafka.client.common.TopicPartition,java.lang.Long>) => {
  results.foreach{
    case (topic: io.vertx.scala.kafka.client.common.TopicPartition, beginningOffset: java.lang.Long) => {
    println(s"End offset for topic=${todo-renderDataObjectMemberSelect}, partition=${todo-renderDataObjectMemberSelect}, beginningOffset=${beginningOffset}")
  }}
})

// Convenience method for single-partition lookup
consumer.endOffsets(topicPartition).onSuccess((endOffset: java.lang.Long) => {
...
consumer.handler((record: io.vertx.scala.kafka.client.consumer.KafkaConsumerRecord<java.lang.String,java.lang.String>) => {
  ...
  if ((record.partition() == 0) && (record.offset() == 5)) {
    ...
    consumer.pause(topicPartition).onSuccess((v: java.lang.Void) => {
      println("Paused")
    }).onSuccess((v: java.lang.Void) => {
...
[source,scala]
----
consumer.close().onSuccess((v: java.lang.Void) => {
  println("Consumer is now closed")
}).onFailure((cause: java.lang.Throwable) => {
  println(s"Close failed: ${cause}")
})
...
for ( i <- 0 until 5) {
  ...
  producer.send(record).onSuccess((recordMetadata: io.vertx.scala.kafka.client.producer.RecordMetadata) => {
...
[source,scala]
----
producer.close().onSuccess((v: java.lang.Void) => {
