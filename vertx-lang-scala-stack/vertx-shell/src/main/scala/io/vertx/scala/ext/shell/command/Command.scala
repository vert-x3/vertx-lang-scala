...
class Command(private val _asJava: Object) {
   ...
   * or  )` method to signal completion is done.   * @param completion the completion object
   */
  ...
  /**
   * Create a new process with the passed arguments.   * @param args the process arguments
   * @return the process
   */
