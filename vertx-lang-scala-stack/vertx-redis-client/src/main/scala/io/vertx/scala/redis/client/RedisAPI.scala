...
class RedisAPI(private val _asJava: Object) {
  ...
  def command(args: scala.collection.mutable.Buffer[String], handler: Handler[AsyncResult[scala.Option[Response]]]): RedisAPI = {
    asJava.asInstanceOf[JRedisAPI].command(args.map(x => x.asInstanceOf[java.lang.String]).asJava, (if (handler == null) null else new io.vertx.core.Handler[AsyncResult[JResponse]]{def handle(x: AsyncResult[JResponse]) {handler.handle(AsyncResultWrapper[JResponse, scala.Option[Response]](x, a => scala.Option(a).map(Response(_))))}}))
    this
  }
