...
class StompServer(private val _asJava: Object) {
   ...
   * it bounds calls the given handler with the result. The result may be a failure if the socket is already used.   * @param handler the handler to call with the result
   * @return the current StompServer
   */
   ...
   * it bounds calls the given handler with the result. The result may be a failure if the socket is already used.   * @param port the port
   ...
   * the result. The result may be a failure if the socket is already used.   * @param port the port
   ...
   * the frames, but also adapt the frame if needed.   * @param handler the handler, must not be `null`
   * @return the current StompServer
   */
...
object StompServer {
  ...
  /**
   * Creates a [[io.vertx.scala.ext.stomp.StompServer]] based on the default Stomp Server implementation, and use the default options.   * @param vertx the vert.x instance to use
   * @return the created StompServer
   */
