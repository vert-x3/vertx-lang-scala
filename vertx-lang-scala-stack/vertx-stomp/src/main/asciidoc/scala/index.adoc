...
Enabling security requires an additional `link:../../scaladocs/io/vertx/scala/ext/auth/authentication/AuthenticationProvider.html[AuthenticationProvider]` handling the
...
More information about `link:../../scaladocs/io/vertx/scala/ext/auth/authentication/AuthenticationProvider.html[AuthenticationProvider]` is available
...
[source,scala]
----
StompClient.create(vertx).connect().onSuccess((connection: io.vertx.scala.ext.stomp.StompClientConnection) => {
  // use the connection
}).onFailure((err: java.lang.Throwable) => {
  println(s"Failed to connect to the STOMP server: ${err.toString()}")
})
...
[source,scala]
----
StompClient.create(vertx).connect(61613, "0.0.0.0").onSuccess((connection: io.vertx.scala.ext.stomp.StompClientConnection) => {
  // use the connection
}).onFailure((err: java.lang.Throwable) => {
  println(s"Failed to connect to the STOMP server: ${err.toString()}")
})
...
[source,scala]
----
StompClient.create(vertx).errorFrameHandler((frame: io.vertx.scala.ext.stomp.Frame) => {
  // Received the ERROR frame
}).connect(61613, "0.0.0.0").onSuccess((connection: io.vertx.scala.ext.stomp.StompClientConnection) => {
  // use the connection
}).onFailure((err: java.lang.Throwable) => {
  println(s"Failed to connect to the STOMP server: ${err.toString()}")
})
...
[source,scala]
----
StompClient.create(vertx, StompClientOptions()
  .setHost("localhost")
  .setPort(1234)
).connect().onSuccess((connection: io.vertx.scala.ext.stomp.StompClientConnection) => {
  // use the connection
}).onFailure((err: java.lang.Throwable) => {
  println(s"Failed to connect to the STOMP server: ${err.toString()}")
})
...
client.connect().onSuccess((connection: io.vertx.scala.ext.stomp.StompClientConnection) => {
  // use the connection
}).onFailure((err: java.lang.Throwable) => {
  println(s"Failed to connect to the STOMP server: ${err.toString()}")
})
...
StompClient.create(vertx).connect().onSuccess((connection: io.vertx.scala.ext.stomp.StompClientConnection) => {
  ...
  connection.send("/queue", Buffer.buffer("Hello")).onSuccess((frame: io.vertx.scala.ext.stomp.Frame) => {
...
[source,scala]
----
StompClient.create(vertx).connect(netClient).onSuccess((connection: io.vertx.scala.ext.stomp.StompClientConnection) => {
  // use the connection
  connection.errorHandler((frame: io.vertx.scala.ext.stomp.Frame) => {
    println(s"ERROR frame received : ${frame}")
  })
...
var http = vertx.createHttpServer(HttpServerOptions()
  .setWebSocketSubProtocols(List("v10.stomp, v11.stomp"))
