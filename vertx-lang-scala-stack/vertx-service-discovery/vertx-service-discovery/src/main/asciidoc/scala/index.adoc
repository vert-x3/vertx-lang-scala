...
discovery.getRecord([
  ...
  if (ar.succeeded() && ar.result() != null) {
    ...
    def client = reference.getAs(Redis.class)
...
When the bridge is registered the `link:../../apidocs/io/vertx/servicediscovery/spi/ServiceImporter.html#start-io.vertx.core.Vertx-io.vertx.servicediscovery.spi.ServicePublisher-io.vertx.core.json.JsonObject-io.vertx.core.Future-[start]` method is called.
