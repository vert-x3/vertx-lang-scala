...
class ServiceImporter(private val _asJava: Object) {
  ...
  /**
   * Closes the importer   * @param closeHandler the handle to be notified when importer is closed, may be `null`
   */
  def close(closeHandler: Handler[Unit]): Unit = {
    asJava.asInstanceOf[JServiceImporter].close((if (closeHandler == null) null else new io.vertx.core.Handler[Void]{def handle(x: Void) {closeHandler.handle(x)}}))
  }


  /**
   * Starts the importer.   * @param vertx the vertx instance
   ...
   */
  def start (vertx: Vertx, publisher: ServicePublisher, configuration: io.vertx.core.json.JsonObject, future: Promise[Unit]): Unit = {
    asJava.asInstanceOf[JServiceImporter].start(vertx.asJava.asInstanceOf[JVertx], publisher.asJava.asInstanceOf[JServicePublisher], configuration, future.asJava.asInstanceOf[JPromise[Void]])
  }
