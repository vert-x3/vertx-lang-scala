...
class ServiceReference(private val _asJava: Object) {
   ...
   * [[io.vertx.scala.servicediscovery.ServiceReference#getAs]] to retrieve the polyglot instance of the object or another facet..   * @return the object to access the service
   */
  def get [T: TypeTag](): T = {
  ...
  /**
   * Gets the object to access the service. It can be a proxy, a client or whatever object. The type depends on the
   * service type and the server itself. This method wraps the service object into the desired type.   * @param x the type of object
   ...
   * Unlike [[io.vertx.scala.servicediscovery.ServiceReference#cached]], this method return the warpped object to the desired (given) type.   * @param x the type of object
  ...
  /**
   * Gets the service object if already retrieved. It won't try to acquire the service object if not retrieved yet.   * @return the object, `null` if not yet retrieved
   */
  ...
  /**
   * Checks whether or not the service reference has the given service object.   * @param object the service object, must not be `null`
