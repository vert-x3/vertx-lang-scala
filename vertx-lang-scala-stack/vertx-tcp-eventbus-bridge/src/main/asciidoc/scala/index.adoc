...
<dependency>
  ...
  <artifactId>vertx-tcp-eventbus-bridge-scala_2.12</artifactId>
...
----
compile 'io.vertx:vertx-tcp-eventbus-bridge-scala_2.12:3.5.1-SNAPSHOT'
----
...
bridge.listenFuture(7000).onComplete{
  case Success(result) => {
    // succeed...
  }
  case Failure(cause) => {
