...
class Context(private val _asJava: Object) {
   ...
   * the verticle was deployed.   * @return the configuration of the deployment or null if not a Verticle deployment
   */
   ...
   * will return true.   * @return true if false otherwise
   */
   ...
   * NOTE! when running blocking code using [[io.vertx.scala.core.Vertx#executeBlocking]] from a
   * standard (not worker) verticle, the context will still an event loop context and this 
   * will return false.   * @return true if the current context is a worker context, false otherwise
   */
