...
class JsonEvent(private val _asJava: Object) {
   ...
   * String value with the key and it contains a Base64 encoded binary, which it decodes if found and returns.   * @return the binary value or `null` if the event has no JSON value
   */
