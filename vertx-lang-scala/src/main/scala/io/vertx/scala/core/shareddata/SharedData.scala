...
class SharedData(private val _asJava: JSharedData) {
  ...
  /**
    * Like [[io.vertx.scala.core.shareddata.SharedData!.getLock(String,io.vertx.lang.scala.AsyncResult):Unit]] but specifying a timeout. If the lock is not obtained within the timeout
