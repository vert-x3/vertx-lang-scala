...
/**
  ...
  * can be replied to using that reply address, or, more simply by just using [[io.vertx.scala.core.eventbus.Message!.reply(message:AnyRef):Unit]].
  * 
  * If you want to notify the sender that processing failed, then [[io.vertx.scala.core.eventbus.Message!.fail(failureCode:Int,message:String):Unit]] can be called.
  */
class Message[T](private val _asJava: JMessage[T]) {
  ...
  /**
    * Link [[io.vertx.scala.core.eventbus.Message!.reply(message:AnyRef):Unit]] but allows you to specify delivery options for the reply.
