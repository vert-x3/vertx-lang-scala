...
class TimeoutStream(private val _asJava: JTimeoutStream) 
  ...
  /**
    * Cancels the timeout. Note this has the same effect as calling [[io.vertx.scala.core.TimeoutStream!.handler(io.vertx.scala.core.Handler[Long]):io.vertx.scala.core.TimeoutStream]] with a null
    * argument.
    */
  def cancel(): Unit = {
    _asJava.cancel()
