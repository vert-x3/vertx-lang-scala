...
class JsonPointerSpec extends AnyFunSpec with Matchers with Inside with ScalaCheckPropertyChecks {
  ...
  describe("JsonPointer") {
    ...
    it("should be constructable from an URI") {
      JsonPointer(new URI("/foo/bar")) shouldBe a[JsonPointer]
      an[IllegalArgumentException] should be thrownBy JsonPointer(new URI("https://vertx.io/docs/vertx-core/java/#_json_pointers"))
    }

    it("should be constructable from an URI, returning Option for safety") {
      JsonPointer.fromURIOption(new URI("/foo/bar")) should matchPattern { case Some(_) => }
      JsonPointer.fromURIOption(new URI("https://vertx.io/docs/vertx-core/java/#_json_pointers")) should matchPattern { case None => }
    }
    ...
    it("should determine if it is a local pointer") {
      ...
      JsonPointer(new URI("/foo/bar")).isLocalPointer should be(false)
    }
