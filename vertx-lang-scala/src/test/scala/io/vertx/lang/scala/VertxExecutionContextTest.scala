...
class SuccessVerticle extends ScalaVerticle {
  override def asyncStart: Future[Unit] = {
    val consumer1Registered = handleInFuture(
      vertx.eventBus.consumer[String]("asd")
                    .handler(println)
                    .completionHandler
    )
    val consumer2Registered = handleInFuture(
      vertx.eventBus
...
class FailVerticle extends ScalaVerticle {
  override def asyncStart: Future[Unit] = {
    val consumerRegistered = handleInFuture(
      vertx.eventBus
        .consumer[String]("asd")
        .handler(a => println(a))
        .completionHandler
  ...
  override def asyncStart: Future[Unit] = handleInFuture(
    vertx.eventBus
      ...
      .handler {
        ...
        case msg @ Message(body) if body == HANDLER_TYPE_VERTXCONTEXT => vertx.runOnContext(_ => msg.reply(Thread.currentThread.getId))
      }
      .completionHandler
  )
  .map(_ => ())
